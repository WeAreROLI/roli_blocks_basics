...
private:
    ...
    static constexpr juce::uint32 numColumns0_byte      = 0;    // 1 byte
    static constexpr juce::uint32 numRows0_byte         = 1;    // 1 byte (ignored for the moment: always square pads to save cycles)
    static constexpr juce::uint32 colours0_byte         = 2;    // 2 byte x 25  (5:6:5 bits for rgb)
    static constexpr juce::uint32 fillTypes0_byte       = 52;   // 1 byte x 25

    static constexpr juce::uint32 numColumns1_byte      = 78;   // 1 byte
    static constexpr juce::uint32 numRows1_byte         = 79;   // 1 byte
    static constexpr juce::uint32 colours1_byte         = 80;   // 2 byte x 25  (5:6:5 bits for rgb)
    static constexpr juce::uint32 fillTypes1_byte       = 130;  // 1 byte x 25

    static constexpr juce::uint32 visiblePads_byte      = 155;  // 1 byte       (i.e. which set of colours/fills to use, 0 or 1)
    static constexpr juce::uint32 slideDirection_byte   = 156;  // 1 byte
    static constexpr juce::uint32 touchedPads_byte      = 158;  // 1 byte x 4   (Zero means empty slot, so stores padIdx + 1)
    static constexpr juce::uint32 animationTimers_byte  = 162;  // 4 byte x 16  (16:16 bits counter:increment)
    static constexpr juce::uint32 totalHeapSize         = 226;

    static constexpr juce::uint32 maxNumPads        = 25;
    static constexpr juce::uint32 colourSizeBytes   = 2;

    int getPadIndex (float posX, float posY) const;
    void setGridFills (int numColumns, int numRows, const juce::Array<GridFill>& fills, juce::uint32 byteOffset);
