...
{
    posX = juce::jmin (0.99f, posX / (float) block.getWidth());
    posY = juce::jmin (0.99f, posY / (float) block.getHeight());

    const juce::uint32 offset  = block.getDataByte (visiblePads_byte) ? numColumns1_byte : numColumns0_byte;
...
void DrumPadGridProgram::startTouch (float startX, float startY)
{
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        ...
        {
            block.setDataByte (touchedPads_byte + i, static_cast<juce::uint8> (padIdx + 1));
            break;
        }
    }
}
...
void DrumPadGridProgram::sendTouch (float x, float y, float z, LEDColour colour)
{
    ...
    e.values[0] = 0x20000000
                    + (juce::jlimit (0, 255, juce::roundToInt (x * (255.0f / (float) block.getWidth())))  << 16)
                    + (juce::jlimit (0, 255, juce::roundToInt (y * (255.0f / (float) block.getHeight()))) << 8)
                    +  juce::jlimit (0, 255, juce::roundToInt (z * 255.0f));

    e.values[1] = (juce::int32) colour.getARGB();
...
{
    juce::uint8 visiblePads = block.getDataByte (visiblePads_byte);

    setGridFills (numColumns, numRows, fills, visiblePads * numColumns1_byte);
}

void DrumPadGridProgram::setGridFills (int numColumns, int numRows, const juce::Array<GridFill>& fills, juce::uint32 byteOffset)
{
    jassert (numColumns * numRows == fills.size());

    block.setDataByte (byteOffset + numColumns0_byte, (juce::uint8) numColumns);
    block.setDataByte (byteOffset + numRows0_byte,    (juce::uint8) numRows);
    ...
    for (auto fill : fills)
    {
        ...
        const juce::uint32 colourOffsetBytes = byteOffset + colours0_byte + i * colourSizeBytes;
        const juce::uint32 colourOffsetBits  = colourOffsetBytes * 8;

        block.setDataBits (colourOffsetBits,      5, (juce::uint32) (fill.colour.getRed()   >> 3));
        block.setDataBits (colourOffsetBits + 5,  6, (juce::uint32) (fill.colour.getGreen() >> 2));
        block.setDataBits (colourOffsetBits + 11, 5, (juce::uint32) (fill.colour.getBlue()  >> 3));

        block.setDataByte (byteOffset + fillTypes0_byte + i, static_cast<juce::uint8> (fill.fillType));
...
{
    juce::uint8 newVisible = block.getDataByte (visiblePads_byte) ? 0 : 1;

    setGridFills (newNumColumns, newNumRows, newFills, newVisible * numColumns1_byte);

    block.setDataByte (visiblePads_byte, newVisible);
    block.setDataByte (slideDirection_byte, (juce::uint8) direction);
}

//==============================================================================
void DrumPadGridProgram::setPadAnimationState (juce::uint32 padIdx, double loopTimeSecs, double currentProgress)
{
    ...
    juce::uint16 aniValue     = juce::uint16 (juce::roundToInt ((255 << 8) * currentProgress));
    juce::uint16 aniIncrement = loopTimeSecs > 0.0 ? juce::uint16 (juce::roundToInt (((255 << 8) / 25.0) / loopTimeSecs)) : 0;

    juce::uint32 offset = 8 * animationTimers_byte + 32 * padIdx;
...
{
    for (juce::uint32 i = 0; i < 16; ++i)
    {
        juce::uint32 offset = 8 * animationTimers_byte + 32 * i;
