...
struct Compiler
{
    ...
    */
    ...
    {
        if (juce::File::isAbsolutePath (include) && juce::File (include).existsAsFile())
            return { include };
    ...
    */
    juce::Array<uint8> compiledObjectCode;
...
private:
    ...
    juce::Array<NativeFunction> nativeFunctions;

    //==============================================================================
    struct CodeLocation
    {
        CodeLocation (const juce::String& code, const juce::File& srcFile) noexcept : program (code), location (program.getCharPointer()), sourceFile (srcFile) {}
        ...
        [[noreturn]] void throwError (const juce::String& message) const
        {
            ...
            auto filePath = sourceFile == juce::File() ? juce::String() : (sourceFile.getFullPathName() + ": ");
            throw filePath + "Line " + juce::String (line) + ", column " + juce::String (col) + " : " + message;
        }

        juce::String program;
        juce::String::CharPointerType location;
    ...
    {
        TokenIterator (const juce::String& code) : location (code, {}), p (code.getCharPointer()) { skip(); }
        ...
        void throwErrorExpecting (const juce::String& expected)    { location.throwError ("Found " + getTokenDescription (currentType) + " when expecting " + expected); }
    ...
    private:
        static bool isIdentifierStart (juce::juce_wchar c) noexcept   { return juce::CharacterFunctions::isLetter (c)        || c == '_'; }
        static bool isIdentifierBody  (juce::juce_wchar c) noexcept   { return juce::CharacterFunctions::isLetterOrDigit (c) || c == '_'; }
        ...
        TokenType matchNextToken()
        {
            if (isIdentifierStart (*p))
            {
                ...
                currentValue = juce::String (p, end); p = end;
            ...
            if (! p.isEmpty())
                location.throwError ("Unexpected character '" + juce::String::charToString (*p) + "' in source");
        ...
        {
            if (p.compareUpTo (juce::CharPointer_ASCII (name), (int) len) != 0) return false;
        ...
        void skipWhitespaceAndComments()
        {
            for (;;)
            {
                ...
                if (*p == '/')
                {
                    ...
                    if (c2 == '/')  { p = juce::CharacterFunctions::find (p, (juce::juce_wchar) '\n'); continue; }

                    if (c2 == '*')
                    {
                        location.location = p;
                        p = juce::CharacterFunctions::find (p + 2, juce::CharPointer_ASCII ("*/"));
        ...
        bool parseStringLiteral (juce::juce_wchar quoteType)
        {
            if (quoteType != '"' && quoteType != '\'')
                return false;

            auto r = juce::JSON::parseQuotedString (p, currentValue);
        ...
        bool parseHexLiteral()
        {
            ...
            auto t = ++p;
            auto v = juce::CharacterFunctions::getHexDigitValue (*++t);
            if (v < 0) return false;

            for (;;)
            {
                auto digit = juce::CharacterFunctions::getHexDigitValue (*++t);
                if (digit < 0) break;
                v = v * 16 + digit;
            }
        ...
        bool parseFloatLiteral()
        {
            ...
            currentValue = juce::CharacterFunctions::getDoubleValue (p);  p = t;
            return true;
        }
        ...
        bool parseOctalLiteral()
        {
            ...
            {
                auto digit = (int) (*++t - '0');
                if (juce::isPositiveAndBelow (digit, 8))        v = v * 8 + digit;
                else if (juce::isPositiveAndBelow (digit, 10))  location.throwError ("Decimal digit in octal constant");
                else break;
            }
        ...
        bool parseDecimalLiteral()
        {
            ...
            {
                auto digit = (int) (*p - '0');
                if (juce::isPositiveAndBelow (digit, 10))  v = v * 10 + digit;
                else break;
            }
    ...
    {
        SyntaxTreeBuilder (const juce::String& code, const juce::Array<NativeFunction>& nativeFns, uint32 defaultHeapSize, const juce::Array<juce::File>& searchPathsToUse)
        ...
        juce::Array<Function*> functions;
        juce::Array<juce::File> searchPaths;
        juce::Array<juce::int64> includedSourceCode;
        const juce::Array<NativeFunction>& nativeFunctions;
    ...
    private:
        juce::OwnedArray<AllocatedObject> allAllocatedObjects;
        ...
        juce::File resolveIncludePath (juce::String include)
        {
        ...
        void parseGlobalVariableDeclaraion (bool isConst, Type type, juce::String name)
        {
            for (;;)
            {
        ...
        void parseGlobalArray (int& arraySize, Type type, const juce::String& name, Variable* parent)
        {
        ...
        void parseGlobalVariable (bool isConst, Type type, juce::String name)
        {
            juce::var constantInitialiser;
        ...
        juce::var parseConstantExpressionInitialiser (Type expectedType)
        {
            juce::var result;
            match (Token::assign);
        ...
        void parseFunctionDeclaration (Type returnType, const juce::String& name)
        {
        ...
        ExpPtr parseFunctionCall (const juce::String& name)
        {
    ...
    {
        CodeGenerator (juce::Array<juce::uint8>& output, SyntaxTreeBuilder& stb)
        ...
        int nextMarker = 0;
        juce::Array<MarkerAndAddress> markersToResolve, resolvedMarkers;
        ...
        void emitPush (const juce::var& value)
        {
            if (value.isDouble())
            {
                const float v = value;
    ...
    struct Expression  : public Statement
    {
        ...
        virtual juce::String getIdentifier() const { location.throwError ("This operator requires an assignable variable"); return {}; }
    };

    struct Variable
    {
        // VS2015 requires a constructor to avoid aggregate initialization
        Variable (const juce::String& n, Type t, bool isGlobalVar, bool isConstVar, const juce::var& cv,
    ...
    struct Function  : public AllocatedObject
    {
        ...
        juce::Array<Type> getArgumentTypes() const
        {
            juce::Array<Type> argTypes;
        ...
        static int countMaxNumLocalVariables (StatementPtr s) noexcept
        {
            struct Counter : Statement::Visitor
            {
                ...
                {
                    num = juce::jmax (num, countMaxNumLocalVariables (sub));
                }
    ...
    struct BlockStatement  : public Statement
    {
        ...
        {
            jassert (requiredType == Type::void_); juce::ignoreUnused (requiredType);
        ...
        int getVariableDepth (const juce::String& name, const CodeLocation& locationForError) const
        {
        ...
        Variable getArray (const juce::String& name, const CodeLocation& locationForError) const
        {
        ...
        int getArrayStart (const juce::String& name, const CodeLocation& locationForError) const
        {
            int start = 0;
        ...
        const juce::Array<Variable>& getGlobalVariables() const noexcept  { return parentBlock != nullptr ? parentBlock->getGlobalVariables() : variables; }
        const juce::Array<Variable>& getGlobalConstants() const noexcept  { return parentBlock != nullptr ? parentBlock->getGlobalConstants() : constants; }
        const juce::Array<Variable>& getGlobalArrays() const noexcept     { return parentBlock != nullptr ? parentBlock->getGlobalArrays() : arrays; }

        const Variable& getVariable (const juce::String& name, const CodeLocation& locationForError) const
        {
        ...
        static int indexOf (const juce::Array<Variable>& vars, const juce::String& name) noexcept
        {
        ...
        Function* function;
        juce::Array<StatementPtr> statements;
        juce::Array<Variable> variables, constants, arrays;
    ...
    {
        LiteralValue (const CodeLocation& l, BlockPtr parent, const juce::var& v) noexcept : Expression (l, parent), value (v) {}
        ...
        void emit (CodeGenerator& cg, Type requiredType, int) const override
        {
            if (requiredType != Type::void_)
            {
                ...
                if (type != requiredType && value != juce::var ((int) 0))
                {
    ...
    {
        Identifier (const CodeLocation& l, BlockPtr parent, const juce::String& n) noexcept : Expression (l, parent), name (n) {}
    ...
    struct FunctionCall  : public Expression
    {
        ...
        void emitArgs (CodeGenerator& cg, const juce::Array<Type>& argTypes, int stackDepth) const
        {
            for (int i = arguments.size(); --i >= 0;)
            {
                ...
                if (argValue->getType (cg) != argType)
                    location.throwError ("Argument " + juce::String (i + 1) + " requires an expression of type " + getTypeName (argType));
    ...
    //==============================================================================
    static juce::Array<Type> getArgTypesFromFunctionName (const char* nameAndTypes)
    {
        juce::Array<Type> types;
        auto args = juce::String (nameAndTypes).fromFirstOccurrenceOf ("/", false, false).substring (1);
    ...
    static FunctionID createFunctionID (juce::String name, Type returnType, const juce::Array<Type>& types)
    {
    ...
    static juce::String getTokenDescription (TokenType t)    { return t[0] == '$' ? juce::String (t + 1) : ("'" + juce::String (t) + "'"); }

    static juce::String getTypeName (Type t) noexcept
    {
    ...
    static Type getTypeOfVar (const juce::var& v) noexcept
    {
