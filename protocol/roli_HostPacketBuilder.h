...
struct HostPacketBuilder
{
    ...
    bool endDataChanges (bool isLastChange) noexcept
    {
        ...
        data << DataChangeCommand ((juce::uint32) isLastChange ? endOfChanges : endOfPacket);
        return true;
    }
    ...
    bool skipBytes (int numToSkip) noexcept
    {
        ...
        if (numToSkip > ByteCountFew::maxValue)
        {
            ...
            data << DataChangeCommand ((juce::uint32) skipBytesMany) << ByteCountMany ((juce::uint32) numToSkip);
            return true;
        }
        ...
        data << DataChangeCommand ((juce::uint32) skipBytesFew) << ByteCountFew ((juce::uint32) numToSkip);
        return true;
    }
    ...
    bool setMultipleBytes (const juce::uint8* values, int num) noexcept
    {
        ...
        data << DataChangeCommand ((juce::uint32) setSequenceOfBytes);

        for (int i = 0; i < num; ++i)
            data << ByteValue ((juce::uint32) values[i])
    ...
    bool setMultipleBytes (juce::uint8 value, juce::uint8 lastValue, int num) noexcept
    {
        ...
        if (num > ByteCountFew::maxValue)
        {
            ...
            data << DataChangeCommand ((juce::uint32) setManyBytesWithValue)
                 << ByteCountMany ((juce::uint32) num)
                 << ByteValue ((juce::uint32) value);
        ...
        if (value == lastValue)
        {
            ...
            data << DataChangeCommand ((juce::uint32) setFewBytesWithLastValue) << ByteCountFew ((juce::uint32) num);
            return true;
        }
        ...
        data << DataChangeCommand ((juce::uint32) setFewBytesWithValue) << ByteCountFew ((juce::uint32) num)
             << ByteValue ((juce::uint32) value);
    ...
    bool addProgramEventMessage (const juce::int32* messageData)
    {
        ...
        for (juce::uint32 i = 0; i < numProgramMessageInts; ++i)
            data << IntegerWithBitSize<32> ((juce::uint32) messageData[i]);
    ...
    bool addFirmwareUpdatePacket (const juce::uint8* packetData, juce::uint8 size)
    {
        ...
        for (juce::uint8 i = 0; i < size; ++i)
            data << IntegerWithBitSize<7> ((juce::uint32) packetData[i]);
    ...
    bool addConfigSetMessage (juce::int32 item, juce::int32 value)
    {
        ...
        data << type << IntegerWithBitSize<8> ((juce::uint32) item) << IntegerWithBitSize<32>((juce::uint32) value);
        return true;
    }
    ...
    bool addRequestMessage (juce::int32 item)
    {
        ...
        data << type << IntegerWithBitSize<32> (0) << IntegerWithBitSize<8> ((juce::uint32) item);
        return true;
    }
    ...
    bool addSetBlockName (const juce::String& name)
    {
        ...
        data << IntegerWithBitSize<7> ((juce::uint32) name.length());

        for (auto i = 0; i < name.length(); ++i)
            data << IntegerWithBitSize<7> ((juce::uint32) name.toRawUTF8()[i]);
...
private:
    ...
    {
        data << MessageType ((juce::uint32) type);
    }
};
