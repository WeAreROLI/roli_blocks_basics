...
public:
    ...
    bool sendCommandMessage (juce::uint32 commandID)
    {
    ...
    void handleCustomMessage (Block::Timestamp, const juce::int32* data)
    {
        ProgramEventMessage m;

        for (juce::uint32 i = 0; i < BlocksProtocol::numProgramMessageInts; ++i)
            m.values[i] = data[i];
    ...
    //==============================================================================
    std::function<void (const Block& block, const juce::String&)> logger;

    void setLogger (std::function<void (const Block& block, const juce::String&)> newLogger) override
    {
        logger = std::move (newLogger);
    }

    void handleLogMessage (const juce::String& message) const
    {
        if (logger != nullptr)
            logger (*this, message);
    }
    ...
    //==============================================================================
    juce::Result setProgram (std::unique_ptr<Program> newProgram,
                             ProgramPersistency persistency = ProgramPersistency::setAsTemp) override
    {
        ...
        if (doProgramsMatch)
        {
            if (isProgramLoaded)
            {
                juce::MessageManager::callAsync ([blockRef = Block::Ptr (this), this]
                {
    ...
    juce::Result loadProgram (ProgramPersistency persistency)
    {
        stopTimer();
        ...
        programSize = (juce::uint32) compiler.compiledObjectCode.size();
    ...
    juce::Result compileProgram()
    {
        ...
        if (compiler.getCompiledProgram().getTotalSpaceNeeded() > getMemorySize())
            return juce::Result::fail ("Program too large!");
    ...
    juce::uint32 getMemorySize() override
    {
        return modelData.programAndHeapSize;
    }

    juce::uint32 getHeapMemorySize() override
    {
        jassert (juce::isPositiveAndNotGreaterThan (programSize, modelData.programAndHeapSize));
        return modelData.programAndHeapSize - programSize;
    }

    void setDataByte (size_t offset, juce::uint8 value) override
    {
    ...
    {
        remoteHeap.setBytes (programSize + offset, static_cast<const juce::uint8*> (newData), num);
    }

    void setDataBits (juce::uint32 startBit, juce::uint32 numBits, juce::uint32 value) override
    {
        remoteHeap.setBits (programSize * 8 + startBit, numBits, value);
    }

    juce::uint8 getDataByte (size_t offset) override
    {
        return remoteHeap.getByte (programSize + offset);
    }

    void handleSharedDataACK (juce::uint32 packetCounter) noexcept
    {
        pingFromDevice();
        remoteHeap.handleACKFromDevice (*this, packetCounter);
    }

    bool sendFirmwareUpdatePacket (const juce::uint8* data, juce::uint8 size, std::function<void (juce::uint8, juce::uint32)> callback) override
    {
    ...
    void handleFirmwareUpdateACK (juce::uint8 resultCode, juce::uint32 resultDetail)
    {
    ...
    void addDataInputPortListener (DataInputPortListener* listener) override
    {
        ...
        {
            {
                juce::ScopedLock scopedLock (deviceConnection->criticalSecton);
    ...
    {
        return juce::RelativeTime::milliseconds (isMaster ? masterPingIntervalMs : dnaPingIntervalMs);
    }

    //==============================================================================
    void handleConfigItemChanged (const ConfigMetaData& data, juce::uint32 index)
    {
    ...
    void factoryReset() override
    {
        ...
        juce::Timer::callAfterDelay (5, [ref = juce::WeakReference<BlockImplementation>(this)]
        {
            if (ref != nullptr)
                ref->blockReset();
        });
    ...
    static constexpr juce::uint32 maxBlockSize = BlocksProtocol::padBlockProgramAndHeapSize;
    static constexpr juce::uint32 maxPacketCounter = BlocksProtocol::PacketCounter::maxValue;
    static constexpr juce::uint32 maxPacketSize = 200;
...
private:
    ...
    juce::uint32 programSize = 0;

    std::function<void (juce::uint8, juce::uint32)> firmwarePacketAckCallback;
...
public:
                                         ...
                                         private Timer
    {
        ...
        void timerCallback() override
        {
            ...
            static const juce::uint32 touchTimeOutMs = 500;
        ...
        struct TouchStatus
        {
            juce::uint32 lastEventTime = 0;
            float lastStrikePressure = 0;
            bool isActive = false;
        };

        void killTouch (const TouchSurface::Touch& touch, TouchStatus& status, juce::uint32 timeStamp) noexcept
        {
    ...
    struct ControlButtonImplementation  : public ControlButton
    {
        ...
        bool setLightColour (LEDColour colour) override
        {
            if (hasLight())
            {
                ...
                {
                    row->setButtonColour ((juce::uint32) buttonIndex, colour);
                    return true;
                }
            }
                                   ...
                                   private Timer
    {
        ...
        void setButtonColour (juce::uint32 index, LEDColour colour)
        {
            if (index < 10)
            {
    ...
    private:
        ...
        {
            if (block.getProgram() != nullptr)
                for (juce::uint32 i = 0; i < (juce::uint32) juce::numElementsInArray (colours); ++i)
                    write565Colour (16 * i, colours[i]);
        }

        void write565Colour (juce::uint32 bitIndex, LEDColour colour)
        {
            block.setDataBits (bitIndex,      5, (juce::uint32) (colour.getRed()   >> 3));
            block.setDataBits (bitIndex + 5,  6, (juce::uint32) (colour.getGreen() >> 2));
            block.setDataBits (bitIndex + 11, 5, (juce::uint32) (colour.getBlue()  >> 3));
        }
