                    ...
                    private juce::AsyncUpdater
{
    ...
    void handleDevicesUpdated (const juce::Array<DeviceInfo>& infos)
    {
    ...
    juce::Array<Block::UID> getDnaDependentDeviceUIDs (Block::UID uid)
    {
        JUCE_ASSERT_MESSAGE_THREAD

        juce::Array<Block::UID> dependentDeviceUIDs;
    ...
    void handleSharedDataACK (Block::UID deviceID, juce::uint32 packetCounter) const
    {
        JUCE_ASSERT_MESSAGE_MANAGER_IS_LOCKED
    ...
    void handleFirmwareUpdateACK (Block::UID deviceID, juce::uint8 resultCode, juce::uint32 resultDetail)
    {
        if (auto* bi = getBlockImplementationWithUID (deviceID))
            bi->handleFirmwareUpdateACK (resultCode, resultDetail);
    }

    void handleConfigUpdateMessage (Block::UID deviceID, juce::int32 item, juce::int32 value, juce::int32 min, juce::int32 max)
    {
        if (auto* bi = getBlockImplementationWithUID (deviceID))
            bi->handleConfigUpdateMessage (item, value, min, max);
    }

    void notifyBlockOfConfigChange (BlockImpl& bi, juce::uint32 item)
    {
    ...
    void handleConfigSetMessage (Block::UID deviceID, juce::int32 item, juce::int32 value)
    {
        if (auto* bi = getBlockImplementationWithUID (deviceID))
        {
            bi->handleConfigSetMessage (item, value);
            notifyBlockOfConfigChange (*bi, juce::uint32 (item));
        }
    }
    ...
    void handleLogMessage (Block::UID deviceID, const juce::String& message) const
    {
        JUCE_ASSERT_MESSAGE_MANAGER_IS_LOCKED
    ...
    void handleButtonChange (Block::UID deviceID, Block::Timestamp timestamp, juce::uint32 buttonIndex, bool isDown) const
    {
        JUCE_ASSERT_MESSAGE_MANAGER_IS_LOCKED
        ...
        if (auto* bi = getBlockImplementationWithUID (deviceID))
        {
            ...
            if (juce::isPositiveAndBelow (buttonIndex, bi->getButtons().size()))
    ...
    void handleCustomMessage (Block::UID deviceID, Block::Timestamp timestamp, const juce::int32* data)
    {
        if (auto* bi = getBlockImplementationWithUID (deviceID))
